...
input_filename='FullTaxa.genus.percent.txt'
output_filebase='tree_genus.json'
header_cols=6
...
for col in range(header_cols,len(input_data[0])):
	...
	for row in range(1,len(input_data)):
		...
		# if this is a zero value, we don't want to add the row
		if (input_data[row][col] == "0"):
			print "zero value";
		
		# if it is not a zero value, traverse the heirarchy and add it
		else:
	...
	# write out 
	with open('./tree-pruned-genus/'+output_filename, 'w') as out_file:
